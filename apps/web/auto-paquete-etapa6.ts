...
async function run() {
  console.log("=== INICIANDO ETAPA VI DESDE CERO (280 PAQUETES TOTALES) ===");
  ...
  const events = [
    { motivo: 1, desc: 'CORTE DEL SERVICIO DE INTERNET', iteraciones: 35 },
    { motivo: 2, desc: 'INACCESIBILIDAD AL SERVICIO WEB', iteraciones: 35 },
    { motivo: 3, desc: 'INGRESO A ZONAS SIN INTERNET', iteraciones: 35 },
    { motivo: 4, desc: 'VENTA EN LUGARES SIN INTERNET', iteraciones: 35 }
  ];

  let numFacturaGlobal = 700000;
  const cantidadFacturas = 5; 

  for (const pv of [1, 0]) {
    console.log(`\n==============================================`);
    console.log(`=== INICIANDO 140 PRUEBAS PARA PV ${pv} ===`);
    console.log(`==============================================\n`);
    ...
    for (const event of events) {
      ...
      for (let p = 1; p <= event.iteraciones; p++) {
        ...
        let validado = false;
        while(iter < 15) {
            await delay(2000); // 2 segundos, mas margen al SIAT
            iter++;
            try {
  ...
  console.log("\n=== REPETICION ETAPA VI COMPLETADA 100% ===");
}
