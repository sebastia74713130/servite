...
async function run() {
  console.log("=== INICIANDO ETAPA VI: PAQUETES (CORTE INTERNET, 500 FACTURAS) ===");
  ...
  let numFacturaGlobal = 200000;
  const cantidadFacturas = 500;

  for (const pv of [1, 0]) {
    console.log(`\n==============================================`);
    console.log(`=== INICIANDO 10 PRUEBAS PARA PV ${pv} ===`);
    console.log(`==============================================\n`);
    ...
    console.log(`   CUFD obtenido. Registrando Evento...`);
    ...
    for (let p = 1; p <= 10; p++) {
      console.log(`   -> Generando Paquete ${p}/10 (500 facturas)...`);
