...
async function run() {
  console.log("=== INICIANDO PRUEBAS VIRUS INFORMATICO (MOTIVO 5) PARA PV 0 ===");
  console.log("Requerimientos: 10 pruebas x PV 0 | < 500 facturas c/u");
  ...
  const evento = { motivo: 5, desc: 'VIRUS INFORMÁTICO O FALLA DE SOFTWARE', iteraciones: 10 };
  const cantidadFacturas = 5; // Menor a 500, muy rapido
  let numFacturaGlobal = 0;
  ...
  for (let p = 1; p <= evento.iteraciones; p++) {
    const tempDir = path.join(process.cwd(), 'scratch', `temp_virus_pv${pv}_p${p}`);
    ...
    for (let f = 0; f < cantidadFacturas; f++) {
        ...
        const xmlStr = buildFacturaXml({
            cabecera: {
                ...
                cafc: "101C155D9178E" // Incluimos el CAFC para que SIAT lo valide
            },
    ...
    const tarName = `paquete_virus_pv${pv}_p${p}.tar.gz`;
    ...
    if (!validado && iter >= 10) process.stdout.write(`TIMEOUT\n`);
  }
  
  console.log("\n=== PRUEBAS PV 0 COMPLETADAS ===");
}
