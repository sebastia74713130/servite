...
async function run() {
  console.log("=== INICIANDO PRUEBAS FALLA DE HARDWARE (MOTIVO 6) ===");
  console.log("Test 11: 10 pruebas x PV 1 | = 500 facturas c/u");
  console.log("Test 12: 10 pruebas x PV 0 | < 500 facturas c/u");
  ...
  const evento = { motivo: 6, desc: 'CAMBIO DE INFRAESTRUCTURA DE SISTEMA O FALLA DE HARDWARE', iteraciones: 10 };
  
  const configurations = [
      { pv: 1, cantidadFacturas: 500 }, // Test 11
      { pv: 0, cantidadFacturas: 5 }    // Test 12 (< 500)
  ];
  ...
  for (const config of configurations) {
    ...
    for (let p = 1; p <= evento.iteraciones; p++) {
      const tempDir = path.join(process.cwd(), 'scratch', `temp_hw_pv${pv}_p${p}`);
      ...
      const tarName = `paquete_hw_pv${pv}_p${p}.tar.gz`;
