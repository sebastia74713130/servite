import { NextResponse } from "next/server";
import { sincronizarActividades } from "@/lib/siat/services/sincronizarCatalogos";

export async function GET(request: Request) {
  try {
    const { searchParams } = new URL(request.url);
    const codigoPuntoVenta = searchParams.get("codigoPuntoVenta") 
      ? parseInt(searchParams.get("codigoPuntoVenta") as string, 10) 
      : 0;
      
    const cuis = searchParams.get("cuis") || process.env.SIAT_CUIS;

    if (!cuis) {
      return NextResponse.json({ success: false, message: "Falta el CUIS" }, { status: 400 });
    }

    const response = await sincronizarActividades({
      codigoAmbiente: 2, 
      codigoPuntoVenta,
      codigoSucursal: 0,
      cuis,
    });
    
    return NextResponse.json({
      success: true,
      message: "Catálogo de Actividades sincronizado exitosamente.",
      data: response,
    });
  } catch (error: any) {
    return NextResponse.json(
      {
        success: false,
        message: "Error al sincronizar catálogos del SIAT.",
        error: error.message || error.toString(),
      },
      { status: 500 }
    );
  }
}
