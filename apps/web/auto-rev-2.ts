...
async function run() {
  console.log("=== INICIANDO RECUPERACIÓN: 2 REVERSIONES PV 1 ===");
  ...
  for (let i = 1; i <= 2; i++) {
    ...
    process.stdout.write(`Prueba ${i}/2 -> `);
    ...
    try {
        ...
        if (emitResult.RespuestaServicioFacturacion?.codigoEstado !== 908) {
            process.stdout.write(`[ERROR EMISION]\n`); continue;
        }
    ...
    try {
        ...
        if (!anulResult.RespuestaServicioFacturacion?.transaccion) {
            process.stdout.write(`[ERROR ANULACION]\n`); continue;
        }
    ...
    try {
        ...
        if (revResult.RespuestaServicioFacturacion?.transaccion) {
            ...
            process.stdout.write(`[ERROR REVERSION]\n`);
        }
