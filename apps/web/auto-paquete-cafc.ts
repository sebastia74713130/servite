...
async function run() {
  console.log("=== INICIANDO PRUEBAS DE FACTURACION CON CAFC ===");
  
  const CAFC_CODE = "101C155D9178E";
  console.log(`Usando CAFC: ${CAFC_CODE}`);
  ...
  const events = [
    { motivo: 2, desc: 'INACCESIBILIDAD AL SERVICIO WEB', iteraciones: 44 },
    { motivo: 3, desc: 'INGRESO A ZONAS SIN INTERNET', iteraciones: 43 },
    { motivo: 4, desc: 'VENTA EN LUGARES SIN INTERNET', iteraciones: 43 }
  ];

  let numFacturaGlobal = 0; // Se incrementará y estará en el rango 1 al 1000
  const cantidadFacturas = 2; // Menor a 500, súper rápido

  for (const pv of [1, 0]) {
    console.log(`\n==============================================`);
    console.log(`=== INICIANDO PRUEBAS PARA PV ${pv} ===`);
    console.log(`==============================================\n`);
    ...
    for (const event of events) {
      console.log(`\n--- PROBANDO MOTIVO ${event.motivo}: ${event.desc} (${event.iteraciones} PAQUETES) ---`);
      ...
      for (let p = 1; p <= event.iteraciones; p++) {
        const tempDir = path.join(process.cwd(), 'scratch', `temp_cafc_pv${pv}_ev${event.motivo}`);
        if (fs.existsSync(tempDir)) fs.rmSync(tempDir, { recursive: true, force: true });
        fs.mkdirSync(tempDir, { recursive: true });

        for (let f = 0; f < cantidadFacturas; f++) {
            numFacturaGlobal++;
            if (numFacturaGlobal > 1000) {
                throw new Error("Límite de CAFC superado (> 1000)");
            }
        ...
        const tarName = `paquete_cafc_pv${pv}_ev${event.motivo}_p${p}.tar.gz`;
            ...
            try {
                const [recepResult] = await compraVentaClient.recepcionPaqueteFacturaAsync({
                    SolicitudServicioRecepcionPaquete: {
                        ...
                        cafc: CAFC_CODE, cantidadFacturas: cantidadFacturas, codigoEvento: codigoEvento
                    }
                });
                if (recepResult.RespuestaServicioFacturacion?.transaccion) {
                    ...
                    console.log(`\nError enviando:`, recepResult.RespuestaServicioFacturacion);
                }
            } catch(e) {}
            await delay(1000);
        }
        ...
        while(iter < 10) {
            ...
            try {
                ...
                if (codDesc === 'VALIDADA') {
                    ...
                    process.stdout.write(`RECH (${JSON.stringify(valResult.RespuestaServicioFacturacion.mensajesList)}) `);
                    break;
                }
            } catch(e) {}
        }
        if (!validado && iter >= 10) process.stdout.write(`T/O `);
        
        if (p % 10 === 0) console.log("");
      }
    }
  }
  console.log("\n=== PRUEBAS CON CAFC COMPLETADAS ===");
}
