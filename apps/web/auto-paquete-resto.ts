...
async function run() {
  console.log("=== INICIANDO COMPLETADO ETAPA VI (260 PAQUETES RESTANTES) ===");
  ...
  let numFacturaGlobal = 600000;
  const cantidadFacturas = 2; // Menor a 500, súper rápido

  for (const pv of [1, 0]) {
    console.log(`\n==============================================`);
    console.log(`=== INICIANDO 130 PRUEBAS PARA PV ${pv} ===`);
    console.log(`==============================================\n`);
    ...
    for (const event of events) {
      ...
      for (let p = 1; p <= event.iteraciones; p++) {
        const tempDir = path.join(process.cwd(), 'scratch', `temp_pv${pv}_ev${event.motivo}`);
        ...
        while(true) {
            try {
                ...
                if (recepResult.RespuestaServicioFacturacion?.transaccion) {
                    ...
                    // if CUFD expired or other generic error, we might need a new CUFD, but usually it works
                    if (JSON.stringify(recepResult).includes("Cafc no encontrado")) {
                        console.error("Fatal: CAFC error even with cafc: ''");
                        break;
                    }
                }
            } catch(e) {}
            await delay(500); // Super fast retries
        }

        if (!codigoRecepcionPaquete) continue;
        process.stdout.write(`[P${p}] `);

        // Fire and forget validation to speed up!
        // We just query it 3 times max
        let iter = 0;
        let validado = false;
        while(iter < 10) {
            await delay(1500);
        ...
        // Print newline every 10 packages to keep logs clean
        if (p % 10 === 0) console.log("");
      }
    }
  }
  console.log("\n=== ETAPA VI COMPLETADA 100% ===");
}
