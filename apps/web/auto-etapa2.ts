...
async function run() {
  console.log("=== COMPLETANDO ETAPA II (Y III) ===");
  ...
  console.log("\\n--- ETAPA II: SINCRONIZACIÓN (1000 CASOS RESTANTES) ---");
  const baseReq = {
  ...
  for (let iter = 1; iter <= 60; iter++) {
      for (const method of syncMethods) {
          try {
              ...
              syncCount++;
              if (syncCount % 50 === 0) process.stdout.write(`Sync ${syncCount}/1020... `);
          } catch(e) {}
      }
      await delay(200);
  }
  ...
  // Top up Etapa III: OBTENCIÓN CUFD
  console.log("\\n--- ETAPA III: CUFD (TOP UP) ---");
  ...
  for (let i = 1; i <= 15; i++) {
      for (const pv of [0, 1]) {
          try {
              ...
              if (cufdCount % 5 === 0) process.stdout.write(`CUFD ${cufdCount}/30... `);
          } catch(e) {}
      }
  }
