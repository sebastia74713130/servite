...
async function run() {
  console.log("=== INICIANDO EVENTOS: RECUPERACIÓN MOTIVO 3 (PV1) ===");
  ...
  const event = { pv: 1, motivo: 3, desc: 'INGRESO A ZONAS SIN INTERNET POR DESPLIEGUE DE PUNTO DE VENTA' };
  ...
  if (!cuis) { console.error(`Fallo crítico al obtener CUIS. Saltando...`); return; }
  ...
  if (!cufd) { console.error(`Fallo crítico al obtener CUFD. Saltando...`); return; }
