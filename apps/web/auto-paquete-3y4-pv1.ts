...
async function run() {
    console.log("=== INICIANDO PRUEBAS: MOTIVOS 3 Y 4, PV 1, 500 FACTURAS ===");
    ...
    for (const event of events) {
        console.log(`\n--- PROBANDO MOTIVO ${event.motivo}: ${event.desc} ---`);
        ...
        for (let p = 1; p <= iteraciones; p++) {
            const tempDir = path.join(process.cwd(), 'scratch', `temp_pv${pv}_ev${event.motivo}_esp2`);
            ...
            const tarName = `paquete_pv${pv}_ev${event.motivo}_esp2_p${p}.tar.gz`;
